...
class BotavioCache {
    ...
    ensureValueType(value, type) {
        if (value !== null) {
            ...
            if (actualType !== type) {
                throw new Error(`Value type mismatch. Expected: ${type}, Actual: ${actualType}`);
            }
        }
    }
    ...
    getValueDescriptor(key) {
        ...
        const valueDescriptor = sValueDescriptor === null ? null : JSON.parse(sValueDescriptor);
    ...
    getValue(key, type) {
        ...
        if (type !== valueDescriptor.type) {
            throw new Error(`Value type mismatch. Expected: ${type}, Actual: ${valueDescriptor.type}`);
        }
